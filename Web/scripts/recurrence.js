...
function Recurrence(recurOptions, recurElements, prefix) {
    ...
    var e = {
        ...
        repeatDate: $('#' + prefix + 'date-RepeatDate'),
    ...
    {
        AddRepeatDate($('#' + prefix + 'formattedRepeatDate').val(), $('#' + prefix + 'date-RepeatDate').val());
    };
    ...
    var DisplayRepeatDates = function() {
        ...
        repeatDates.sort((r1, r2) => r1.system.localeCompare(r2.system)).forEach((v, i) => {
            datediv.append($("<div data-repeat-date='"  + v.system + "'><span>" + v.user + "</span> <a href='#'><i class='bi bi-x-circle icon delete remove-repeat-date' data-repeat-date='" + v.system +"'></i></a><input type='hidden' name='repeatCustomDates[]' value='" + v.system +"'</div>"));
        });
    };
