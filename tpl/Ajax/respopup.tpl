...
{if $authorized}
    <div class="res_popup_details" style="margin:0">
        ...
        {capture "name"}
            <div class="user">
                {if ($hideUserInfo || $hideDetails) && !$showUserName}
        ...
        {capture "participants"}
            {if !$hideUserInfo && !$hideDetails && count($participants) > 0}
                <div class="users">
        ...
        {capture "coowners"}
            {if !$hideUserInfo && !$hideDetails && count($coowners) > 0}
                <div class="coowners">
                    {translate key="CoOwners"} ({$coowners|@count}):
                    {foreach from=$coowners item=user name=coowner_loop}
                        {fullname first=$user->FirstName last=$user->LastName}
                        {if !$smarty.foreach.coowner_loop.last}, {/if}
                    {/foreach}
                </div>
            {/if}
        {/capture}
        {$formatter->Add('coowners', $smarty.capture.coowners)}

        {capture "accessories"}
            {if !$hideDetails && count($accessories) > 0}
                <div class="accessories">
                    {translate key="Accessories"} ({count($accessories)}):
        ...
        {capture "description"}
            {if !$hideDetails}
                <div class="summary">{if $summary neq ''}{nl2br($summary|truncate:300:"...")}{else}{translate key=NoDescriptionLabel}{/if}</div>
            {/if}
        {/capture}
