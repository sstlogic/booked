...
{function name=displaySlot}
    {call name=$DisplaySlotFactory->GetFunction($Slot, true) Slot=$Slot ResourceId=$ResourceId SlotRef=$SlotRef}
{/function}
    ...
    <div>
        <table id="reservations-{formatdate date=$date format='Y-m-d'}" data-date="{formatdate date=$date format='Y-m-d'}" class="reservations" border="1" cellpadding="0" width="100%">
            ...
            {foreach from=$Resources item=resource name=resource_loop}
                ...
                <tr class="slots">
                    ...
                    {foreach from=$slots item=slot}
                        {assign var=slotRef value="{$slot->BeginDate()->Format('YmdHis')}{$resource->GetId()}"}
                        {displaySlot Slot=$slot ResourceId=$resource->GetId() SlotRef=$slotRef}
                    {/foreach}
                </tr>
            {/foreach}
            ...
            <tr class="slots">
                ...
                {foreach from=$slots item=slot}
                    {assign var=slotRef value="{$slot->BeginDate()->Format('YmdHis')}{$User->UserId*-1}"}
                    {displaySlot Slot=$slot ResourceId=$User->UserId*-1 SlotRef=$slotRef}
                {/foreach}
            </tr>
            ...
            {foreach from=$Participants item=participant name=participant_loop}
                ...
                <tr class="slots">
                    ...
                    {foreach from=$slots item=slot}
                        {assign var=slotRef value="{$slot->BeginDate()->Format('YmdHis')}{$participant->UserId*-1}"}
                        {displaySlot Slot=$slot ResourceId=$participant->UserId*-1 SlotRef=$slotRef}
                    {/foreach}
                </tr>
            {/foreach}
            ...
            {foreach from=$Invitees item=invitee name=participant_loop}
                ...
                <tr class="slots">
                    ...
                    {foreach from=$slots item=slot}
                        {assign var=slotRef value="{$slot->BeginDate()->Format('YmdHis')}{$invitee->UserId*-1}"}
                        {displaySlot Slot=$slot ResourceId=$invitee->UserId*-1 SlotRef=$slotRef}
                    {/foreach}
                </tr>
            {/foreach}
            </tbody>
