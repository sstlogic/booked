...
class ResourceDisplayPresenter extends ActionPresenter
{
    ...
    public function DisplayResource($resourcePublicId)
    {
        ...
        $reservations = $this->reservationService->GetReservations($reservationSearchRange, null, $timezone, $resource->GetResourceId(), true);
        ...
        $dailyLayout = $this->dailyLayoutFactory->Create(new EmptyReservationListing(), $layout);
        ...
        foreach ($reservationList as $r) {
            $dtos = $r->AsDto(new NullUserSession(), $reservationSearchRange);
            foreach ($dtos as $dto) {
                $reservationsAsDtos[] = $dto;
            }
            ...
            if (!$resource->GetCheckinLimitedToAdmins() && $r->RequiresCheckin()) {
        ...
        if ($requiresCheckin && count($current) > 1) {
            $requiresCheckin = false;
            $checkinReferenceNumber = "";
        }

        $this->SetTermsOfService();
        $this->page->SetIsAvailableNow(count($current) < $resource->GetMaxConcurrentReservations());
        $this->page->DisplayAvailability($dailyLayout, $now, $current, $next, $upcoming, $requiresCheckin, $checkinReferenceNumber, $reservationsAsDtos);
    }
    ...
    private function GetHandler($userSession)
    {
        if ($this->reservationCreateHandler == null) {
            ...
            return ReservationHandler::Create($reservationAction, $factory->Create($reservationAction), $userSession);
        }
    ...
    private function GetCheckinHandler($userSession)
    {
        if ($this->reservationCheckinHandler == null) {
            $factory = new ReservationPersistenceFactory();
            $reservationAction = ReservationAction::Checkin;
            return ReservationHandler::Create($reservationAction, $factory->Create($reservationAction), $userSession);
        }
