...
class ManageReservationsPresenter extends ActionPresenter
{
    ...
    public function PageLoad($userTimezone)
    {
        ...
        $pageUserType = $this->page->GetUserType();
        $userType = ReservationUserLevel::OWNER;
        if ($pageUserType == ReservationUserLevel::PARTICIPANT) {
            $userType = ReservationUserLevel::PARTICIPANT;
        }
        if ($pageUserType == ReservationUserLevel::CO_OWNER) {
            $userType = ReservationUserLevel::CO_OWNER;
        }
        ...
        if (!$this->page->FilterButtonPressed()) {
            ...
            $userType = $filterPreferences->GetFilterUserType();
            ...
            $filterPreferences->SetFilterUserType($userType);
        ...
        $this->page->SetUserType($userType);
        ...
        $filter = new ReservationFilter($startDate, $endDate, $referenceNumber, $scheduleId, $resourceId, $userId,
            $userType, $reservationStatusId, $resourceStatusId, $resourceReasonId, $attributeFilters, $title, $description, $missedCheckin, $missedCheckout);
        ...
        $this->page->SetIsFiltered($filter->IsFiltered());
    ...
    public function UpdateAttribute()
    {
        ...
        Log::Debug('Updating reservation attribute.',
            ['userId' => $userSession->UserId, 'attributeId' => $attributeId, 'value' => $attributeValue, 'referenceNumber' => $referenceNumber]);
    ...
    public function ImportReservations()
    {
        ...
        for ($i = 0; $i < count($rows); $i++) {
            ...
            try {
                ...
                Log::Error('Error importing reservations.', ['exception' => $ex]);
            }
        }
    ...
    public function DeleteMultiple()
    {
        ...
        Log::Debug('Reservation multiple delete.', ['ids' => $ids]);
        foreach ($ids as $id) {
...
class ReservationFilterPreferences
{
    ...
    private $FilterUserType = ReservationUserLevel::OWNER;
    ...
    public function GetFilterUserType()
    {
    ...
    public function SetFilterUserType($FilterUserType)
    {
    ...
    static $filterKeys = ['FilterStartDateDelta' => -7,
