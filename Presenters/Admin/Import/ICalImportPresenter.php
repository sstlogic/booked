...
class ICalImportPresenter extends ActionPresenter
{
    ...
    public function Import()
    {
        ...
        Log::Debug('Found events in ics import file', ['count' => count($events)]);
        ...
        foreach ($events as $event) {
            try {
                ...
                Log::Debug('Importing reservation', ['start' => $start, 'end' => $end, 'location' => $location]);

                $this->reservationRepository->Add($reservation);
                $numberImported++;
            } catch (Exception $ex) {
                Log::Error('Error importing event from ICS', ['exception' => $ex]);
            }
        }

        Log::Debug('Done running ics import.', ['countImported' => $numberImported, 'countSkipped' => $numberSkipped]);
