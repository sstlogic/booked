...
{if $CreditsCurrent > 0}
    <div>
        Tämä varaus maksaa {$CreditsCurrent} krediittiä.
        {if $CreditsCurrent != $CreditsTotal}
            Koko varaussarja maksaa {$CreditsTotal} krediittiä.
        {/if}
    </div>
{/if}

{if !empty($CreatedBy)}
    <div><strong>Luonut:</strong> {$CreatedBy}</div>
{/if}

{if !empty($ApprovedBy)}
    <div><strong>Hyväksynyt:</strong> {$ApprovedBy}</div>
{/if}
