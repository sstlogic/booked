...
class ResourceResponse extends RestResponse
{
    ...
    public function __construct(IRestServer $server, $resource, $attributes)
    {
        ...
        $this->name = apidecode($resource->GetName());
        $this->location = apidecode($resource->GetLocation());
        $this->contact = apidecode($resource->GetContact());
        $this->notes = apidecode($resource->GetNotes());
        ...
        $this->description = apidecode($resource->GetDescription());
        ...
        $this->creditsChargedAllSlots = $resource->GetCreditsAlwaysCharged();
        $this->maxConcurrentReservations = $resource->GetMaxConcurrentReservations();
        $this->minParticipants = $resource->GetMinParticipants();
        $this->slotLabel = apidecode($resource->GetSlotLabel());
        $this->adminGroupId = $resource->GetAdminGroupId();
        $this->extendIfMissedCheckout = $resource->IsAutoExtendEnabled();
        $this->checkinLimitedToAdmins = $resource->GetCheckinLimitedToAdmins();
        $this->autoReleaseAction = $resource->GetAutoReleaseAction();
...
class ExampleResourceResponse extends ResourceResponse
{
    public function __construct()
    {
        ...
        $this->creditsChargedAllSlots = true;
        $this->autoReleaseAction = "1 (delete) or 2 (end reservation) or null";
