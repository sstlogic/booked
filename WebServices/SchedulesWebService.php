...
class SchedulesWebService
{
     ...
     * @return Response
     */
    public function GetSchedules(Request $request, Response $response, $args)
    {
        $schedules = $this->scheduleRepository->GetAll();

        return $this->server->WriteResponse(new SchedulesResponse($this->server, $schedules), $response);
    }
     ...
     * @return Response
     */
    public function GetSchedule(Request $request, Response $response, $args)
    {
        ...
        if ($schedule != null) {
            ...
            return $this->server->WriteResponse(new ScheduleResponse($this->server, $schedule, $layout), $response);
        } else {
     ...
     * @return Response
     */
    public function GetSlots(Request $request, Response $response, $args)
    {
        $scheduleId = $args['scheduleId'];
        ...
        $s = $this->GetDate($request, WebServiceQueryStringKeys::START_DATE_TIME);
        $e = $this->GetDate($request, WebServiceQueryStringKeys::END_DATE_TIME);
        if ($s) {
            $startDate = $s->ToTimezone($tz);
        }
        if ($e) {
            $endDate = $e->ToTimezone($tz);
        }

        $resourceId = $this->server->GetQueryString($request, WebServiceQueryStringKeys::RESOURCE_ID);
        ...
        if ($isError) {
            return $this->server->WriteResponse(RestResponse::Unauthorized(), $response, RestResponse::UNAUTHORIZED_CODE);
        } else {
            $resp = new ScheduleSlotsResponse($this->server, $scheduleId, $layout, $dates, $resources, $this->privacyFilter);
            return $this->server->WriteResponse($resp, $response);
        }
    }

    private function GetDate(Request $request, $queryStringKey)
    {
