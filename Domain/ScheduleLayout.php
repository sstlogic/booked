...
class ScheduleLayout implements IScheduleLayout, ILayoutCreation
{
	...
	public function GetSlotCount(Date $startDate, Date $endDate)
	{
		...
		return new SlotCount($slots, $peakSlots, $unreservableSlots);
	}
...
class SlotCount
{
	...
	public function __construct($offPeak, $peak, $blocked)
	{
...
class PeakTimes
{
	...
	public function IsWithinPeak(Date $date)
	{
		...
		if ($date->Compare($peakStart) >= 0 && $date->Compare($peakEnd) <= 0)
		{
			...
			{
				Log::Debug('Date is within peak', ['date' => $date, 'peakStart' => $peakStart, 'peakEnd' => $peakEnd]);
				return true;
			}
		}

        Log::Debug('Date not is within peak', ['date' => $date, 'peakStart' => $peakStart, 'peakEnd' => $peakEnd]);
		return false;
	}
