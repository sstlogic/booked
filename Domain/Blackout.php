...
class BlackoutSeries
{
     ...
     */
    public function Update($ownerId, $scope, $title, $blackoutDate, $repeatOptions, $resourceIds, $timezone)
    {
        ...
        if ($scope == SeriesUpdateScope::ThisInstance) {
            ...
            $currentDate = $this->CurrentBlackout()->Date()->ToTimezone($timezone);
    ...
    public function Delete(Blackout $blackout)
    {
        if (count($this->blackouts) <= 1) {
            Log::Debug('Only blackout in the series. Cannot delete.', ['id' => $blackout->Id()]);
            return false;
        }
        ...
        Log::Debug('Deleted blackout', ['id' => $blackout->Id()]);
        return true;
    }
