...
class ReportCommandBuilder
{
    ...
    const RESERVATION_LIST_FRAGMENT = '`rs`.`date_created` as `date_created`, `rs`.`last_modified` as `last_modified`, `rs`.`repeat_type`, `rs`.`description` as `description`, `rs`.`delete_reason` as `delete_reason`, `rs`.`title` as `title`, `rs`.`status_id` as `status_id`,
		`ri`.`reference_number`, `ri`.`start_date`, `ri`.`end_date`, `ri`.`checkin_date`, `ri`.`checkout_date`, `ri`.`previous_end_date`, `ri`.`credit_count`, TIMESTAMPDIFF(SECOND, `ri`.`start_date`, `ri`.`end_date`) as `duration`,
                            ...
                            (SELECT GROUP_CONCAT(CONCAT(`coowner_users`.`fname`, " ", `coowner_users`.`lname`) SEPARATOR "!sep!")
								FROM `reservation_users` `coowners` INNER JOIN `users` `coowner_users` ON `coowner_users`.`user_id` = `coowners`.`user_id` WHERE `coowners`.`reservation_instance_id` = `ri`.`reservation_instance_id` AND `coowners`.`reservation_user_level` = 4 ORDER BY `lname`, `fname`) as `coowner_list`,
							(SELECT GROUP_CONCAT(CONCAT(`cav`.`custom_attribute_id`,\'=\', `cav`.`attribute_value`) SEPARATOR "!sep!")
    ...
    const DURATION_FRAGMENT = '`ri`.`start_date`, `ri`.`end_date`, TIMESTAMPDIFF(SECOND, `ri`.`start_date`, `ri`.`end_date`) as `duration`';
    ...
    const PARTICIPANT_JOIN_FRAGMENT = 'INNER JOIN `users` `participants` ON `participants`.`user_id` IN (@participant_id)
    ...
    const COOWNER_JOIN_FRAGMENT = 'INNER JOIN `users` `coowners` ON `coowners`.`user_id` IN (@coowner_id)
			INNER JOIN `reservation_users` `cu` ON `cu`.`user_id` = `coowners`.`user_id` AND `cu`.`reservation_user_level` = 4 AND `cu`.`reservation_instance_id` = `ri`.`reservation_instance_id` ';
    ...
    const USER_ID_FRAGMENT = 'AND `owner`.`user_id` IN (@userid)';
     ...
     * @return ReportCommandBuilder
     */
    public function WithCoOwnerIds($userIds)
    {
        $this->joinCoOwners = true;
        $this->coOwnerIds = is_array($userIds) ? $userIds : [$userIds];
        return $this;
    }
     ...
     * @return ReportCommandBuilder
     */
    public function WithParticipantIds($userIds)
    {
        $this->joinParticipants = true;
        $this->participantIds = is_array($userIds) ? $userIds : [$userIds];
        return $this;
    }
     ...
     * @return ReportCommandBuilder
     */
    public function WithReservationAttributes($attributes, $customAttributes) {
        $this->attributes = $attributes;
        $this->customAttributes = [];
        foreach ($customAttributes as $attribute) {
            $this->customAttributes[$attribute->Id()] = $attribute;
        }
        return $this;
    }
    ...
    private function GetJoin()
    {
        ...
        if ($this->joinCoOwners) {
            $join->Append(self::COOWNER_JOIN_FRAGMENT);
        }
    ...
    private function GetWhereAnd()
    {
        ...
        if (!empty($this->userIds)) {
            ...
            $this->AddParameter(new Parameter(ParameterNames::USER_ID, $this->userIds));
        }

        if (!empty($this->coOwnerIds)) {
            $this->AddParameter(new Parameter(ParameterNames::COOWNER_ID, $this->coOwnerIds));
        }

        if (!empty($this->participantIds)) {
            $this->AddParameter(new Parameter(ParameterNames::PARTICIPANT_ID, $this->participantIds));
        }
        ...
        if (!empty($this->customAttributes) && !empty($this->attributes)) {
            $attributes = [];
            foreach ($this->attributes as $a) {
                if (array_key_exists($a->AttributeId, $this->customAttributes)) {
                    $attributes[] = new \Booked\Attribute($this->customAttributes[$a->AttributeId], $a->Value);
                }
            }
            $attributeFilter = AttributeFilter::Create(TableNames::RESERVATION_SERIES_ALIAS . '.' . ColumnNames::SERIES_ID, $attributes);

            if ($attributeFilter != null) {
                $and->Append('AND '  . $attributeFilter->__toString());
                foreach ($attributeFilter->Criteria() as $c) {
                    $this->AddParameter(new Parameter($c->Variable, $c->Value));
                }
            }
        }
     ...
     * @return bool
     */
    public function GetIncludeDuration()
    {
        return $this->duration || $this->fullList;
    }

    /**
     * @return bool
     */
    public function GetIncludeTotalTime() {
        return $this->time;
    }
}
