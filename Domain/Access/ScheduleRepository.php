...
class ScheduleRepository implements IScheduleRepository
{
    ...
    public function LoadDefaultSchedule()
    {
        $reader = ServiceLocator::GetDatabase()->Query(new GetDefaultScheduleCommand());

        if ($row = $reader->GetRow()) {
            $schedule = Schedule::FromRow($row);
        } else {
            throw new Error("No default schedule");
        }

        $reader->Free();

        $this->_cache->Add($schedule->GetId(), $schedule);
        return $schedule;
    ...
    public function Update(Schedule $schedule)
    {
        ServiceLocator::GetDatabase()->Execute(new UpdateScheduleCommand(
            ...
            $schedule->GetAllowBlockedEndSlot()));
    ...
    public function GetLayout($scheduleId, ILayoutFactory $layoutFactory)
    {
        ...
        while ($row = $reader->GetRow()) {
            ...
            if (!empty($row[ColumnNames::BLOCK_START]) && !empty($row[ColumnNames::BLOCK_END])) {
