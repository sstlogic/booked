...
require_once(ROOT_DIR . 'Domain/ReservationConflictView.php');
...
interface IReservationViewRepository
{
     ...
     */
    public function GetList($pageNumber, $pageSize, $sortField = null, $sortDirection = null, $filter = null, $userLevel = null);
     ...
     * @param int|null $scheduleId
     * @return ReservationConflictView[]
     */
    public function GetConflicts(DateRange $dateRange, $resourceIds = null, $scheduleId = null, $includeBlackouts = false);
}
...
class ReservationViewRepository implements IReservationViewRepository
{
    ...
    public function GetReservationForEditing($referenceNumber)
    {
        ...
        while ($row = $reader->GetRow()) {
            ...
            $reservationView->CheckinLimitedToAdmin = (bool)$row[ColumnNames::CHECKIN_LIMITED_TO_ADMINS];
            ...
            if (array_key_exists(ColumnNames::RESERVATION_MEETING_LINK_TYPE, $row) && !empty($row[ColumnNames::RESERVATION_MEETING_LINK_TYPE])) {
                $reservationView->MeetingLink = new ReservationMeetingLinkView($row[ColumnNames::RESERVATION_MEETING_LINK_TYPE], $row[ColumnNames::RESERVATION_MEETING_LINK_URL]);
            }
        ...
        while ($row = $reader->GetRow()) {
            if ($consolidateByReferenceNumber) {
                ...
                if (array_key_exists($refNum, $reservations)) {
                    ...
                    $reservations[$refNum]->ResourceIds[] = $row[ColumnNames::RESOURCE_ID];
                } else {
    ...
    public function GetList($pageNumber, $pageSize, $sortField = null, $sortDirection = null, $filter = null, $userLevel = null)
    {
        $command = new GetFullReservationListCommand($userLevel);
    ...
    private function SetParticipants(ReservationView $reservationView)
    {
        ...
        while ($row = $reader->GetRow()) {
            ...
            if ($levelId == ReservationUserLevel::CO_OWNER) {
                $reservationView->CoOwners[] = $reservationUserView;
            }
    ...
    private function SetAccessories(ReservationView $reservationView)
    {
        ...
        while ($row = $reader->GetRow()) {
            $reservedQuantity = $row[ColumnNames::QUANTITY];
            $accessoryQuantity = $row[ColumnNames::ACCESSORY_QUANTITY];
    ...
    public function GetConflicts(DateRange $dateRange, $resourceIds = null, $scheduleId = null, $includeBlackouts = false)
    {
        ...
        $reservationsCommand = new GetConflictingReservationsCommand($dateRange->GetBegin(), $dateRange->GetEnd(), $resourceIds, $scheduleId);
        $reader = ServiceLocator::GetDatabase()->Query($reservationsCommand);
        while ($row = $reader->GetRow()) {
            $conflicts[] = new ReservationConflictView(
                ...
                $row[ColumnNames::RESOURCE_ID],
                $row[ColumnNames::RESOURCE_NAME],
                empty($row[ColumnNames::RESOURCE_BUFFER_TIME]) ? null : TimeInterval::Parse($row[ColumnNames::RESOURCE_BUFFER_TIME])
            );
        }
        ...
        if ($includeBlackouts) {
            $blackoutsCommand = new GetConflictingBlackoutsCommand($dateRange->GetBegin(), $dateRange->GetEnd(), $resourceIds, $scheduleId);
            $reader = ServiceLocator::GetDatabase()->Query($blackoutsCommand);
            while ($row = $reader->GetRow()) {
                $id = $row[ColumnNames::BLACKOUT_INSTANCE_ID];
                $start = Date::FromDatabase($row[ColumnNames::BLACKOUT_START]);
                $end = Date::FromDatabase($row[ColumnNames::BLACKOUT_END]);
                $resourceId = $row[ColumnNames::RESOURCE_ID];
                $userId = $row[ColumnNames::USER_ID];
                $scheduleId = $row[ColumnNames::SCHEDULE_ID];
                $resourceName = $row[ColumnNames::RESOURCE_NAME];
                $seriesId = $row[ColumnNames::BLACKOUT_SERIES_ID];
                $conflicts[] = new ReservationConflictView($id, $seriesId, '', $start, $end, $resourceId, $resourceName, null);
            }
            $reader->Free();
        }
