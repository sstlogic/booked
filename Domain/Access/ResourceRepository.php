...
class ResourceRepository implements IResourceRepository
{
    ...
    {
        $filter = new SqlFilterIn(new SqlFilterColumn(TableNames::RESOURCES_ALIAS, ColumnNames::RESOURCE_ID), $resourceIds);
        $command = new FilterCommand(new GetAllResourcesCommand(), $filter);
    ...
    private function LoadResource($command)
    {
        ...
        if ($row = $reader->GetRow()) {
            ...
            $getResourceRelationshipsCommand = new GetResourceRelationshipsCommand($resource->GetId());
            $resourceRelationshipReader = ServiceLocator::GetDatabase()->Query($getResourceRelationshipsCommand);
            while ($relationshipRow = $resourceRelationshipReader->GetRow()) {
                if ($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_TYPE] == ResourceRelationshipTypes::REQUIRED) {
                    $resource->WithRequiredRelationship($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_ID1]);
                    $resource->WithRequiredRelationship($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_ID2]);
                } elseif ($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_TYPE] == ResourceRelationshipTypes::REQUIRED_ONE_WAY) {
                    $resource->WithRequiredOneWayRelationship($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_ID2]);
                } elseif ($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_TYPE] == ResourceRelationshipTypes::EXCLUDED) {
                    $resource->WithExcludedRelationship($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_ID1]);
                    $resource->WithExcludedRelationship($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_ID2]);
                } elseif ($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_TYPE] == ResourceRelationshipTypes::EXCLUDED_AT_TIME) {
                    $resource->WithExcludedTimeRelationship($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_ID1]);
                    $resource->WithExcludedTimeRelationship($relationshipRow[ColumnNames::RESOURCE_RELATIONSHIP_ID2]);
                }
            }
            $resourceRelationshipReader->Free();
        }
    ...
    public function Update(BookableResource $resource)
    {
        ...
        $updateResourceCommand = new UpdateResourceCommand(
            ...
            $resource->GetCreditsAlwaysCharged(),
            $resource->IsAutoExtendEnabled(),
            $resource->GetCheckinLimitedToAdmins(),
            $resource->GetMinParticipants(),
        ...
        $db->Execute(new DeleteResourceGroupsCommand($resource->GetId()));
        foreach ($resource->GetResourceGroupIds() as $groupId) {
            $db->Execute(new AddResourceToGroupCommand($resource->GetId(), $groupId));
        }

        $requiredRelationships = $resource->GetRequiredRelationshipsDiff();
        $requiredOneWayRelationships = $resource->GetRequiredOneWayRelationshipsDiff();
        $excludedRelationships = $resource->GetExcludedRelationshipsDiff();
        $excludedTimeRelationships = $resource->GetExcludedTimeRelationshipsDiff();

        foreach ($requiredRelationships->GetRemovedFromArray1() as $id) {
            $db->Execute(new DeleteResourceRelationshipCommand($resource->GetId(), $id));
        }
        foreach ($requiredRelationships->GetAddedToArray1() as $id) {
            $db->Execute(new AddResourceRelationshipCommand($resource->GetId(), $id, ResourceRelationshipTypes::REQUIRED));
        }
        foreach ($requiredOneWayRelationships->GetRemovedFromArray1() as $id) {
            $db->Execute(new DeleteResourceRelationshipCommand($resource->GetId(), $id));
        }
        foreach ($requiredOneWayRelationships->GetAddedToArray1() as $id) {
            $db->Execute(new AddResourceRelationshipCommand($resource->GetId(), $id, ResourceRelationshipTypes::REQUIRED_ONE_WAY));
        }
        foreach ($excludedRelationships->GetRemovedFromArray1() as $id) {
            $db->Execute(new DeleteResourceRelationshipCommand($resource->GetId(), $id));
        }
        foreach ($excludedRelationships->GetAddedToArray1() as $id) {
            $db->Execute(new AddResourceRelationshipCommand($resource->GetId(), $id, ResourceRelationshipTypes::EXCLUDED));
        }
        foreach ($excludedTimeRelationships->GetRemovedFromArray1() as $id) {
            $db->Execute(new DeleteResourceRelationshipCommand($resource->GetId(), $id));
        }
        foreach ($excludedTimeRelationships->GetAddedToArray1() as $id) {
            $db->Execute(new AddResourceRelationshipCommand($resource->GetId(), $id, ResourceRelationshipTypes::EXCLUDED_AT_TIME));
        }
    ...
    {
        Log::Debug("Deleting resource", ['resourceId', $resource->GetResourceId(), 'name' => $resource->GetName()]);

        $resourceId = $resource->GetResourceId();

        $db = ServiceLocator::GetDatabase();
        $db->Execute(new DeleteAttributeEntityValuesCommand($resourceId));
    ...
    public function GetResourceGroups($scheduleId = ResourceRepository::ALL_SCHEDULES, $resourceFilter = null)
    {
        ...
        foreach ($this->GetScheduleResources($scheduleId) as $r) {
            ...
            $_assignments[] = new ResourceGroupAssignment(0, $r->GetName(), $r->GetResourceId(), $r->GetAdminGroupId(),
                ...
                $r->GetRequiredRelationships(), $r->GetPublicId(), $r->GetResourceGroupIds(), $r->GetAutoReleaseAction());
        }
        ...
        while ($row = $resources->GetRow()) {
            ...
            if (array_key_exists($resourceId, $resourceList)) {
                ...
                $_assignments[] = new ResourceGroupAssignment($row[ColumnNames::RESOURCE_GROUP_ID], $r->GetName(),
                    ...
                    $r->GetMaxConcurrentReservations(), $r->GetRequiredRelationships(), $r->GetPublicId(), $r->GetResourceGroupIds(), $r->GetAutoReleaseAction());
            }
        }
    ...
    public function ChangeResourceGroupPermission($resourceId, $groupId, $type)
    {
        ...
        if ($type != ResourcePermissionType::None && in_array($type, [ResourcePermissionType::View, ResourcePermissionType::Full])) {
    ...
    public function ClearResourceGroupPermissions($resourceId) {
        ServiceLocator::GetDatabase()->Execute(new DeleteAlGroupResourcePermissions($resourceId));
    }
    ...
    public function ChangeResourceUserPermission($resourceId, $userId, $type)
    {
        ...
        if (in_array($type, [ResourcePermissionType::None, ResourcePermissionType::View, ResourcePermissionType::Full])) {
    ...
    public function ClearResourceUserPermissions($resourceId) {
        ServiceLocator::GetDatabase()->Execute(new DeleteAllUserResourcePermissions($resourceId));
    }
    ...
    {
        $resources = [];
        $command = new GetResourcesByRecentlyUsedCommand($userId, ResourceStatus::AVAILABLE);
        $reader = ServiceLocator::GetDatabase()->Query($command);
        while ($row = $reader->GetRow()) {
            $resources[] = BookableResource::Create($row);
        }
    ...
    {
        $resources = [];
        $command = new GetUserResourceFavoritesCommand($userId, ResourceStatus::AVAILABLE);
        $reader = ServiceLocator::GetDatabase()->Query($command);
        while ($row = $reader->GetRow()) {
            $resources[] = BookableResource::Create($row);
        }
    ...
    {
        $deleteAssignments = new DeleteResourceGroupAssignment($groupId);
        ServiceLocator::GetDatabase()->Execute($deleteAssignments);

        foreach ($resourceIds as $resourceId) {
            $addAssignment = new AddResourceToGroupCommand($resourceId, $groupId);
            ServiceLocator::GetDatabase()->Execute($addAssignment);
        }
    }
}
...
class AccessoryDto
{
     ...
     * @var bool;
     */
    public $CreditsChargedAllSlots = false;
     ...
     * @param bool $creditsChargedAllSlots
     */
                                ...
                                $creditsChargedAllSlots)
    {
        ...
        $this->CreditsChargedAllSlots = $creditsChargedAllSlots;
    }
...
class ResourceDto implements IBookableResource
{
     ...
     * @param int[]|null $requiredResourceIds
     * @param string|null $publicId
     * @param int[]|null $resourceGroupIds
    ...
    )
    {
        ...
        $this->AutoReleaseAction = $autoReleaseAction;
        $this->Color = $color;
        $this->TextColor = '';
        if (!empty($color)) {
        ...
        $this->RequiredResourceIds = empty($requiredResourceIds) ? [] : $requiredResourceIds;
        $this->PublicId = $publicId;
        $this->ResourceGroupIds = empty($resourceGroupIds) ? [] : $resourceGroupIds;
    }
    ...
    /**
     * @var ResourceAutoReleaseAction|int|null
     */
    ...
    {
        // TODO: Implement GetAutoReleaseAction() method.
    }
}
...
class NullResourceDto extends ResourceDto
{
    public function __construct()
    {
        parent::__construct(0, null, false, false, 0, new TimeInterval(0), null, null, null, null, false, false, false, null, null, null, [], null, null);
    }
}