...
class GroupImportCsvRow
{
    ...
    public function IsValid()
    {
        ...
        if (!$isValid) {
            Log::Debug('Group import row is not valid.', ['name' => $this->name]);
        }
        return $isValid;
    }
...
class GroupImportCsv
{
    ...
    public function GetRows()
    {
        ...
        Log::Debug('Found rows in group import file', ['count' => count($csvRows)]);
        ...
        for ($i = 1; $i < count($csvRows); $i++) {
            ...
            if ($row->IsValid()) {
                ...
                Log::Error('Skipped import of group row', ['row' => $i, 'values' => $values]);
