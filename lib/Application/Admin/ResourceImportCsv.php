...
class ResourceImportCsvRow
{
    ...
    public $creditsBlockedSlots = false;
	...
	public function __construct($values, $indexes, $attributes)
	{
        ...
        $this->creditsBlockedSlots = $this->valueOrDefault('creditsBlockedSlots');
		$this->maximumConcurrent = $this->valueOrDefault('concurrentReservations');
		$this->slotLabel = $this->valueOrDefault('slotLabel');
	...
	public static function GetHeaders($values, $attributes)
	{
		...
		$indexes['creditsBlockedSlots'] = self::indexOrFalse('credits are charged for blocked slots', $values);
		$indexes['concurrentReservations'] = self::indexOrFalse('maximum concurrent reservations', $values);
		$indexes['slotLabel'] = self::indexOrFalse('slot label', $values);
...
class ResourceImportCsv
{
	...
	public function GetRows()
	{
		...
		Log::Debug('Found rows in resource import file', ['count' => count($csvRows)]);
		...
		if (!$headers)
		{
			...
			if (count($csvRows) > 0)
            {
                Log::Debug('Resource import header row', ['row' => var_export(str_getcsv($csvRows[0]), true)]);
            }
			return $rows;
		}
		...
		for ($i = 1; $i < count($csvRows); $i++)
		{
			...
			{
				Log::Error('Skipped import of resource row', ['row' => $i, 'values' => $values]);
