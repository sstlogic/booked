...
class ManageReservationsService implements IManageReservationsService
{
								...
								$reservationRepository = null)
	{
		...
		$this->persistenceService = $persistenceService == null ? $persistenceFactory->Create(ReservationAction::Update) : $persistenceService;
	...
	public function LoadFiltered($pageNumber, $pageSize, $sortField, $sortDirection, $filter, $user)
	{
		return $this->reservationViewRepository->GetList($pageNumber, $pageSize, $sortField, $sortDirection, $filter->GetFilter(), $filter->GetUserLevel());
	}
	...
	public function LoadByReferenceNumber($referenceNumber, $user)
	{
		...
		if ($this->reservationAuthorization->CanEdit(ReservationAuthorizationView::ConvertView($reservation), $user))
		{
			return $reservation;
		}
