...
interface IAttributeService
{
     ...
     */
    public function GetAttributes($category, UserSession $userSession, $entityIds = array());
...
class AttributeService implements IAttributeService
{
    ...
    public function GetAttributes($category, UserSession $userSession, $entityIds = array())
    {
        ...
        foreach ($attributes as $attribute) {
            if ($this->AttributeAllowed($attribute, $userSession, $entityIds)) {
...
//        Log::Debug('Took %d seconds to load custom attributes for category %s', $stopwatch->GetTotalSeconds(), $category);
    ...
    public function Validate($category, $attributeValues, $entityIds = array(), $ignoreEmpty = false, $isAdmin = false)
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            $value = array_key_exists($attribute->Id(), $values) ? trim($values[$attribute->Id()]) : null;
        ...
        foreach ($customAttributes as $attribute) {
            ...
            if (empty($secondaryCategory) ||
                ...
                if ($viewableForPrivate && $viewableForAdmin) {
                    $attributes[] = new \Booked\Attribute($attribute, $reservationView->GetAttributeValue($attribute->Id()));
                }
            }
        }
     ...
     * @return bool
     */
    private function AttributeAllowed(CustomAttribute $attribute, UserSession $user, $entityIds)
    {
        $appliesToEntity = false;
        if (!$attribute->UniquePerEntity() || empty($entityIds)) {
            $appliesToEntity = true;
        } else {
            foreach ($entityIds as $eid) {
                if ($attribute->AppliesToEntity($eid)) {
                    $appliesToEntity = true;
                    break;
                }
            }
        ...
        if (!$attribute->AdminOnly() || $user->IsAdmin) {
            ...
            if ($attribute->Category() == CustomAttributeCategory::USER) {
                $isAdminOk = $this->GetAuthorizationService()->IsAdminFor($user, $entityIds[0]);
            }

            if ($attribute->Category() == CustomAttributeCategory::RESOURCE) {
                $isAdminOk = $user->IsResourceAdmin || $user->IsScheduleAdmin;
            }
        }

        return $isAdminOk && $appliesToEntity;
    }
}
