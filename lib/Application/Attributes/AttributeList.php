...
//			Log::Debug('Adding custom attribute definition label=%s', $attribute->Label());
	...
	public function AddValue($attributeEntityValue)
	{
		...
		{
			$this->values[$entityId][$attributeId] = new \Booked\Attribute($this->definitions[$attributeId], $attributeEntityValue->Value);
		}
		elseif ($this->EntityAttributeExists($attributeId, $entityId))
		{
			$this->values[$entityId][$attributeId] = new \Booked\Attribute($this->entityDefinitions[$entityId][$attributeId], $attributeEntityValue->Value);
		}
	}
	...
	public function GetAttributes($entityId = null)
	{
		...
		foreach ($this->attribute_order as $attributeId => $placeholder)
		{
			...
			{
				if (empty($entityId) ||
                    !array_key_exists($entityId, $this->values) ||
                    !array_key_exists($attributeId, $this->values[$entityId])
				)
				{
					$attributes[] = new \Booked\Attribute($definition);
				}
				else
				{
