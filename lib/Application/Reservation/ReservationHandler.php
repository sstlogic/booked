...
class ReservationHandler implements IReservationHandler
{
    ...
    public function Handle($reservationSeries, IReservationSaveResultsView $view)
    {
        ...
        if (!empty($reservationSeries->MeetingLink()) && !empty($reservationSeries->MeetingLink()->Errors()))
        {
            $view->SetWarnings(array_merge($validationResult->GetWarnings(), $reservationSeries->MeetingLink()->Errors()));
        }
        else {
