...
class PreReservationFactory implements IPreReservationFactory
{
	...
	public function CreatePreCheckinService(UserSession $userSession)
	{
		...
		$rules[] = new ReservationCanBeCheckedInRule(new AdminForReservationCheck($userSession, $this->userRepository));
	...
	public function CreatePreCheckoutService(UserSession $userSession)
	{
		...
		$rules[] = new ReservationCanBeCheckedOutRule(new AdminForReservationCheck($userSession, $this->userRepository));
	...
	private function CreateUpdateService(ReservationValidationRuleProcessor $ruleProcessor, UserSession $userSession)
	{
        ...
        $ruleProcessor->AddRule(new AdminExcludedRule(new ReservationPastAutoreleaseRule(), $userSession, $this->userRepository));
	...
	private function GetRuleProcessor(UserSession $userSession)
	{
        ...
        $rules[] = new AdminExcludedRule(new ResourceMinParticipationRule(), $userSession, $this->userRepository);
        $rules[] = new AdminExcludedRule(new PermissionValidationRule(new PermissionServiceFactory()), $userSession, $this->userRepository);
        $rules[] = new MeetingConnectionEstablishedValidationRule($this->userRepository, new MeetingConnectionFactory());
	...
	private function GetAddUpdateRuleProcessor(UserSession $userSession)
	{
        ...
        $ruleProcessor->AddRule(new MeetingLinkRule());
		...
		$ruleProcessor->AddRule(new AdminExcludedRule(new UserConcurrentLimitRule($this->reservationRepository), $userSession, $this->userRepository));
		$ruleProcessor->AddRule(new AdminExcludedRule(new ResourceRelationshipRule($this->resourceRepository, $this->reservationRepository), $userSession, $this->userRepository));
