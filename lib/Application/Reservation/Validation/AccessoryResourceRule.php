<?php
/**
 * Copyright 2014 Twinkle Toes Software, LLC
 */
		...
		foreach ($reservationSeries->AllResources() as $resource)
		{
			...
			if ($association->ContainsResource($resourceId))
			{
				...
				foreach ($resourceAccessories as $accessory)
				{
					...
					if (!empty($resource->MinQuantity) && (!isset($bookedAccessories[$accessoryId]) || $bookedAccessories[$accessoryId]->QuantityReserved < $resource->MinQuantity))
					{
						$errors[] = $this->strings->GetString('AccessoryMinQuantityErrorMessage', array($resource->MinQuantity, $accessory->GetName()));
					}

					if (!empty($resource->MaxQuantity) && isset($bookedAccessories[$accessoryId]) && $bookedAccessories[$accessoryId]->QuantityReserved > $resource->MaxQuantity)
					{
...
class ResourceAccessoryAssociation
{
	...
	private function AccessoryNeedsARequiredResourceToBeBooked($accessoryId, $bookedResourceIds)
	{
		...
		{
			Log::Debug('Checking required resources for accessory', ['accessoryId' => $accessoryId]);
