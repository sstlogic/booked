...
class SlotLabelFactory
{
	...
	public function Format(ReservationItemView $reservation, $format = null)
	{
		...
		$name = ($shouldHideUser && !$showUserName) ? Resources::GetInstance()->GetString('Private') : $this->GetFullName($reservation);
		...
		$label = str_replace('{groups}', implode(', ', $reservation->OwnerGroupNames()), $label);
		if (!$shouldHideUser)
		{
