...
class CalendarReservation
{
    ...
    private static function GetClass(ReservationItemView $reservation)
    {
        ...
        if ($reservation->IsCoOwner($user->UserId)) {
            return 'reserved coowner';
        }
    ...
    private function GetUrl()
    {
        if (!empty($this->ReferenceNumber)) {
            return sprintf('%s?rn=%s&redirect=[redirect]', UrlPaths::RESERVATION, $this->ReferenceNumber);
        }

        if (!empty($this->ResourceId)) {
            $format = Resources::GetInstance()->GetDateFormat('url_full');
            return  sprintf('%s?rid=%s&sd=%s&ed=%s', UrlPaths::RESERVATION, $this->ResourceId, $this->StartDate->Format($format), $this->EndDate->Format($format));
        }

        if (!empty($this->ScheduleId)) {
            $format = Resources::GetInstance()->GetDateFormat('url_full');
            return  sprintf('%s?sid=%s&sd=%s&ed=%s', UrlPaths::RESERVATION, $this->ScheduleId, $this->StartDate->Format($format), $this->EndDate->Format($format));
        }
