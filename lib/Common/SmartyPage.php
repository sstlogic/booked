...
class SmartyPage extends Smarty
{
    ...
    protected function RegisterFunctions()
    {
        ...
        $this->registerPlugin('function', 'javascript_array', array($this, 'CreateJavascriptArray'));
        ...
        $this->registerPlugin('function', 'javascript_boolean', array($this, 'JavascriptBoolean'));
        $this->registerPlugin('function', 'markdown', [$this, 'ToMarkdown']);
    ...
    public function IsValid()
    {
        try {
            ...
            Log::Error('Error during page validation', ['exception' => $ex]);
            return false;
        }
    }
    ...
    public function ValidationGroup($params, $content, $smarty, &$repeat)
    {
        ...
        if (!$repeat) {
            ...
            return empty($actualContent) ? '' :
                "<div class=\"$class validation-feedback\">
					<i class=\"bi bi-exclamation-circle bi-big\"></i>
					<ul class=\"error-list\">$actualContent</ul>
				</div>";
        }
        return '';
    }
    ...
    public function ObjectHtmlOptions($params, $smarty)
    {
        ...
        foreach ($options as $option) {
            ...
            $builder->Append(sprintf('<option label="%s" value="%s"%s>%s</option>', $_label, $_key, $isselected, $_label));
        }
    ...
    public function SetFocus($params, $smarty)
    {
        ...
        return "<script>document.getElementById('$id').focus();</script>";
    }
    ...
    public function CreatePageLink($params, $smarty)
    {
        ...
        $iscurrent = isset($params['iscurrent']) ? $params['iscurrent'] : false;
    ...
    public function DisplayFullName($params, $smarty)
    {
        ...
        if ($config->GetSectionKey(ConfigSection::PRIVACY, ConfigKeys::PRIVACY_ALWAYS_SHOW_USER_NAME, new BooleanConverter())) {
            $ignorePrivacy = true;
        }
    ...
    public function IncludeJavascriptFile($params, $smarty)
    {
        ...
        if (Configuration::Instance()->GetKey(ConfigKeys::USE_LOCAL_JS, new BooleanConverter())) {
            echo "<script src=\"{$this->RootPath}scripts/{$params['src']}?v=$versionNumber\"{$async}></script>";
            return;
        }
        echo "<script src=\"https://cdn.bookedscheduler.com/$versionNumber/scripts/{$params['src']}?v=$versionNumber\"{$async}></script>";
    }
    ...
    public function IncludeCssFile($params, $smarty)
    {
        ...
        echo "<link rel='stylesheet' type='text/css' href='{$this->RootPath}{$src}{$qsSign}v=$versionNumber'/>";
    }

    public function DisplayIndicator($params, $smarty)
    {
        $id = isset($params['id']) ? $params['id'] : '';
        $spinClass = isset($params['spinClass']) ? $params['spinClass'] : 'spinner-border';
//		$size = isset($params['size']) ? "fa-{$params['size']}x" : 'fa-2x';
        ...
        echo "<div id=\"$id\" class=\"$spinClass $class $show\" role=\"status\"><span class=\"visually-hidden\">Working</span></div>";;
...
//		echo "<span id=\"$id\" class=\"fa fa-spin $spinClass $size $class $show\"></span>";
    ...
    public function CancelButton($params, $smarty)
    {
        ...
        echo '<button type="button" class="btn btn-default cancel ' . $class . '" data-bs-dismiss="modal" ' . $this->GetButtonAttributes($params) . '>' .
    ...
    public function UpdateButton($params, $smarty)
    {
        ...
        echo '<button type="' . $type . '" class="btn btn-primary' . $save . $class . '" ' . $this->GetButtonAttributes($params) . '><span class="bi bi-check-circle"></span> ' . Resources::GetInstance()
    ...
    public function DeleteButton($params, $smarty)
    {
        ...
        echo '<button type="' . $type . '" class="btn btn-danger save ' . $class . '" ' . $this->GetButtonAttributes($params) . '><span class="bi bi-trash"></span> ' . Resources::GetInstance()
                ->GetString($key) . '</button>';
    }

    public function ResetButton($params, $smarty)
    {
        $key = isset($params['key']) ? $params['key'] : 'ClearFilter';
        $class = isset($params['class']) ? $params['class'] : '';
    ...
    public function FilterButton($params, $smarty)
    {
        ...
        $class = isset($params['class']) ? $params['class'] : '';
        echo '<button type="submit" class="btn btn-primary ' . $class . '" ' . $this->GetButtonAttributes($params) . '> <span class="bi bi-search"></span> ' . Resources::GetInstance()
                ->GetString($key) . '</button>';
    }
    ...
    public function OkButton($params, $smarty)
    {
        ...
        $class = isset($params['class']) ? $params['class'] : '';
        echo '<button type="button" class="btn btn-success ' . $class . '" ' . $this->GetButtonAttributes($params) . '><span class="bi bi-check-circle"></span> ' . Resources::GetInstance()
                ->GetString($key) . '</button>';
    }
    ...
    {
        $class = isset($params['class']) ? $params['class'] : '';
        echo '<button type="button" class="btn btn-link"><span class="icon black show-hide bi ' . $class . '"></span><span class="no-show">Show/Hide</span></button>';
    }
    ...
    public function SortColumn($params, $smarty)
    {
        ...
        if ($sortField == $currentField) {
            $indicator = "<i class=\"bi bi-sort-down\"></i>";
            if ($currentDirection == 'asc') {
                $indicator = "<i class=\"bi bi-sort-up\"></i>";
            }
        }
     ...
     * @return int
     */
    public function JavascriptBoolean($params, $smarty)
    {
        return ($params['val'] == 1 || $params['val'] == true || $params['val'] == "true") ? "true" : "false";
    }
     ...
     * @return string
     */
    public function ToMarkdown($params, $smarty)
    {
