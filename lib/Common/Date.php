...
class Date
{
    ...
    public function __construct($timestring = null, $timezone = null)
    {
        ...
        $this->date = new DateTime(empty($timestring) ? 'now' : $timestring, new DateTimeZone($this->timezone));
    ...
    public function ToTimezone($timezone)
    {
        try {
            ...
            Log::Error("Error converting timezone.", ['from' => $this->timezone, 'to' =>$timezone]);
            global $GLOBALS;
            if (!in_array($timezone, $GLOBALS)) {
                $timezone = Configuration::Instance()->GetDefaultTimezone();
            }
            $date = new DateTime($this->timestring, new DateTimeZone($this->timezone));
     ...
     * @return string
     */
    public function ToIso($forceUtc = false)
    {
...
//		//die(' off '  .$offset . ' tz ' . $this->date->getTimezone()->getOffset());
        if ($forceUtc) {
            return $this->ToUtc()->Format(DATE_ISO8601);
        }
     ...
     * @return int comparison result
     */
    public static function CompareStatic(Date $date1, Date $date2)
    {
    ...
    private function NeverNull(?int $val): int
    {
        if (empty($val)) {
            return 0;
        }
    ...
    {
        $days = $this->NeverNull($days);
        if ($days < 0) {
            return $this->SubtractDays($days * -1);
        }
    ...
    {
        $weeks = $this->NeverNull($weeks);
        if ($weeks < 0) {
            return $this->SubtractWeeks($weeks * -1);
        }
        return new Date($this->Format(self::SHORT_FORMAT) . " +" . $weeks . " weeks", $this->timezone);
    }
    ...
    {
        $seconds = $this->NeverNull($seconds);
        if ($seconds < 0) {
            return $this->SubtractSeconds($seconds * -1);
        }
        return new Date($this->Format(self::SHORT_FORMAT) . " +" . $seconds . " seconds", $this->timezone);
    }
    ...
    {
        $minutes = $this->NeverNull($minutes);
        if ($minutes < 0) {
...
//        return $utcDate->ToTimezone($this->timezone);
    ...
    {
        return intval($this->parts['hours']);
    }
    ...
    {
        return intval($this->parts['minutes']);
    }
    ...
    {
        return intval($this->parts['seconds']);
    }
...
class DateDiff
{
    ...
    {
        return intval($this->seconds / 3600) - intval($this->Days() * 24);
    }

    public function TotalMinutes()
    {
        return intval(floor($this->seconds / 60));
    }

    public function Minutes()
    {
        return intval(floor(intval(($this->seconds / 60)) % 60));
    }
