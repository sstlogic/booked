...
class Ldap extends Authentication implements IAuthentication
{
     ...
     * @param LdapRecordWrapper $ldapImplementation The actual LDAP implementation to work against
    ...
    public function __construct(IAuthentication $authentication, $ldapImplementation = null, $ldapOptions = null)
    {
        ...
        if ($ldapImplementation == null) {
            $this->ldap = new LdapRecordWrapper($this->options);
        }
    }
    ...
    public function Validate($username, $password)
    {
        ...
        Log::Debug("Result of LDAP Authenticate", ['username' => $username, 'isValid' => $isValid]);
        ...
        if ($isValid) {
            ...
            if (!$userLoaded) {
                Log::Error("Could not load user details from LDAP. Check your ldap settings.", ['username' => $username]);
            }
            return $userLoaded;
        }
        else {
    ...
    private function CleanUsername($username)
    {
        ...
        if (BookedStringHelper::Contains($username, '@')) {
            Log::Debug('LDAP - Username appears to be an email address. Cleaning.', ['username' => $username]);
            $parts = explode('@', $username);
            $username = $parts[0];
        }
        if (BookedStringHelper::Contains($username, '\\')) {
            Log::Debug('LDAP - Username appears contain a domain. Cleaning.', ['username' => $username]);
            $parts = explode('\\', $username);
            $username = $parts[1];
        }
