...
interface IReservationPopupPage
{
	...
	/**
	 * @param $resources ReservationResourceView[]
	 */
	function SetResources($resources);

	/**
	 * @param $users ReservationUserView[]
	 */
	 ...
	 */
	public function SetHideUser($hideUserInfo, $showUserName);
...
class PopupFormatter
{
	...
	public function Display()
	{
		...
		if (empty($label))
		{
			$label = "{pending} {name} {email} {dates} {duration} {title} {resources} {coowners} {participants} {accessories} {description} {attributes}";
		}
		...
		$label = str_replace('{coowners}', $this->GetValue('coowners'), $label);
...
class ReservationPopupPage extends Page implements IReservationPopupPage
{
	...
	public function SetHideUser($hideUserInfo, $showUserName)
	{
		...
		$this->Set('showUserName', $showUserName);
	}
...
class ReservationPopupPresenter
{
	...
	public function PageLoad()
	{
        ...
        $showUserName = Configuration::Instance()->GetSectionKey(ConfigSection::PRIVACY,
																 ConfigKeys::PRIVACY_ALWAYS_SHOW_USER_NAME,
																 new BooleanConverter());
		...
		if ($hideReservationDetails || $hideUserInfo || $showUserName)
		{
			$canViewDetails = $this->_reservationAuthorization->CanViewDetails(ReservationAuthorizationView::ConvertView($reservation), ServiceLocator::GetServer()->GetUserSession());
		...
		$this->_page->SetHideUser($hideUserInfo, $showUserName);
		...
		$this->_page->SetCoOwners($reservation->CoOwners);
