...
interface ISchedulePage extends IActionPage
{
    ...
    /**
     * @param Date $previousWeek
     * @param Date $nextWeek
     * @param bool $showWeeks
     */
    public function SetPreviousNextWeeks($previousWeek, $nextWeek, $showWeeks);
...
//    public function GetParticipantText();
    ...
    /**
     * @param bool $allowTallView
     */
    public function SetAllowTallView($allowTallView);

    /**
     * @param bool $usingAppointments
     */
    public function SetIsUsingAppointments($usingAppointments);
    ...
    /**
     * @param int $visibleDays
     */
    public function SetVisibleDays($visibleDays);
}

class SchedulePage extends ActionPage implements ISchedulePage, IUserAutocompleteApiPage
{
    protected $ScheduleStyle = ScheduleStyle::Standard;
    private $resourceCount = 0;
    private $usingAppointments = false;
    ...
    private $_styles = array(
        ...
        ScheduleStyle::Mobile => 'Schedule/schedule-mobile.tpl',
    );
    ...
    public function __construct()
    {
        ...
        $this->Set('ShowResourceWarning', false);
        $this->Set('HideSchedule', false);
        $this->Set('SubscriptionUrl', '');
        $this->Set('ScheduleAvailabilityEarly', false);
        $this->Set('ScheduleAvailabilityLate', false);
        $this->Set('ShowWeekNumbers', false);
        $this->Set('ResourceIds', []);
        $this->Set('ScheduleId', null);
        $this->Set('FirstDate', null);
        $this->Set('LastDate', null);
        $this->Set('SpecificDates', null);
        $this->Set('MinCapacityFilter', null);
        $this->Set('ResourceTypeIdFilter', null);
        $this->Set('ResourceAttributes', null);
        $this->Set('ResourceTypeAttributes', null);
        $this->Set('Resources', []);
        $this->Set('UserLevelFilter', null);
    ...
    public function ProcessPageLoad()
    {
        ...
        $this->Set('CreateReservationPage', UrlPaths::RESERVATION);
        $this->Set('LoadViewOnly', false);
        $this->Set('ShowSubscription', true);
        $this->Set('UserIdFilter', $this->GetUserId());
        $this->Set('UserLevelFilter', $this->GetUserLevel());
        ...
        if ($this->usingAppointments) {
            $this->SetScheduleStyle(ScheduleStyle::Appointment);
            if ($this->IsMobile) {
                $this->Display('Schedule/schedule-appointment-mobile.tpl');
            } else {
                $this->Display('Schedule/schedule-appointment.tpl');
            }
        } else {
        ...
        Log::Debug('Schedule render times.', ['load' => $load, 'render' => $display, 'total' => $total]);
    }
    ...
    public function SetPreviousNextWeeks($previousWeek, $nextWeek, $showWeeks)
    {
        ...
        $this->Set('ShowWeekButtons', $showWeeks);
    }

    public function GetSelectedDate()
    {
        $matches = array();
        $var = $this->server->GetQuerystring(QueryStringKeys::START_DATE);
        preg_match('/\d{4}-\d{1,2}-\d{1,2}/', $var . '', $matches);
        return count($matches) > 0 ? $matches[0] : $var;
    }
    ...
    public function GetResourceIds()
    {
        $resourceIds = $this->GetQuerystring('rid');
...
//        $participantText = $this->GetParticipantText();
        ...
        $this->Set('ResourceIds', empty($resourceFilter->ResourceIds) ? [] : $resourceFilter->ResourceIds);

        $this->Set('UserId', $ownerId);
//        if (!empty($ownerId)) {
//            $this->Set('UserText', $ownerText);
//        }
...
//        }
    ...
    public function GetReservationRequest()
    {
        ...
        return $builder
            ...
            ->WithUser($this->GetForm(FormKeys::USER_ID))
            ->WithUserLevel($this->GetForm(FormKeys::USER_LEVEL))
            ->Build();
    }
    ...
    public function BindReservations($items, $displayRange)
    {
        $showCheckin = Configuration::Instance()->GetSectionKey(ConfigSection::SCHEDULE, ConfigKeys::SCHEDULE_SHOW_CHECKIN, new BooleanConverter());
        $itemsAsJson = [];
        foreach ($items as $item) {
            $dtos = $item->AsDto($this->server->GetUserSession(), $displayRange);
            foreach ($dtos as $dto) {
                if (!$showCheckin) {
                    $dto->IsMissedCheckIn = false;
                    $dto->IsMissedCheckOut = false;
                    $dto->IsCheckedOut = false;
                    $dto->IsCheckedIn = false;
                }
                $itemsAsJson[] = $dto;
            }
        }
    ...
    public function GetUserLevel()
    {
        $level = $this->GetQuerystring(FormKeys::USER_LEVEL);
        if (empty($level)) {
            return null;
        }
    ...
    public function SetAllowTallView($allowTallView)
    {
        $this->Set('TallViewAllowed', $allowTallView);
    }

    public function SetIsUsingAppointments($usingAppointments)
    {
        $this->usingAppointments = $usingAppointments;
        $this->Set('UsingAppointments', $usingAppointments);
    }

    public function GetDaysVisible()
    {
        $dv = $this->GetQuerystring(QueryStringKeys::DAYS_VISIBLE);
        $cookie = $this->server->GetCookie(CookieKeys::DAYS_VISIBLE);
        if (!empty($dv) && is_numeric($dv)) {
            return intval($dv);
        }
    ...
    public function SetVisibleDays($visibleDays)
    {
        $this->Set('VisibleDays', $visibleDays);
        $this->server->SetCookie(new Cookie(CookieKeys::DAYS_VISIBLE, $visibleDays));
    }
